  ...
  return (
    <div className="about-page">
      ...
      <p style={{ color: 'var(--text-dim)', marginBottom: 16 }}>
        Setup guide and training walkthrough for new Corvus operators.
      </p>
...
cd corvus`}</CodeBlock>
        ...
        <StepCard number={2} title="Set up PostgreSQL">
          ...
          <CodeBlock>{`sudo -u postgres psql -c "CREATE USER corvus WITH PASSWORD 'corvus';"
sudo -u postgres psql -c "CREATE DATABASE corvus OWNER corvus;"
sudo -u postgres psql -c "GRANT ALL PRIVILEGES ON DATABASE corvus TO corvus;"`}</CodeBlock>
        </StepCard>
...
npm run build`}</CodeBlock>
      ...
      <section className="about-section">
        ...
        <p>
          Corvus can be used as an MCP server, allowing Claude Code to query the neuron graph directly
          ...
          <CodeBlock>{`# .mcp.json (already in repo root)
{
  "mcpServers": {
    "corvus": {
      "type": "stdio",
      "command": "/home/tylerbvogel/Projects/corvus/backend/venv/bin/python",
      "args": ["-m", "app.mcp_server"],
      "cwd": "/home/tylerbvogel/Projects/corvus/backend"
    }
  }
}`}</CodeBlock>
          <p style={{ marginTop: 8 }}>
            When you open Claude Code in the <code>corvus/</code> directory, it will automatically
            detect this file and make the Corvus tools available.
          </p>
        </StepCard>
        ...
        <StepCard number={10} title="Test MCP tools in Claude Code">
          ...
          <CodeBlock>{`cd ~/Projects/corvus
claude
# Then ask: "Use the corvus graph_stats tool to show me the neuron graph statistics"`}</CodeBlock>
          <p style={{ marginTop: 8 }}>
        ...
        <StepCard number={11} title="Use Corvus context in queries">
          <p>
...
│   │       └── corvus_org.yaml # Domain hierarchy (9 depts)
│   ├── tests/                   # pytest suite
│   └── requirements.txt
├── frontend/
│   ├── src/
      ...
      <section className="about-section">
        ...
        <table className="about-table" style={{ fontSize: '0.8rem' }}>
          ...
          <tbody>
            ...
            <tr><td><code>DATABASE_URL</code></td><td><code>postgresql+asyncpg://corvus:corvus@localhost:5432/corvus</code></td><td>PostgreSQL connection string</td></tr>
...
function CorvusSetupGuide() {
  ...
  const runHealthCheck = async () => {
    ...
    try {
      // Check 1: Is the Corvus service reachable within Corvus?
      ...
      if (statusRes.ok) {
        ...
        result.message = 'Corvus endpoint returned an error. Check that Corvus is running on port 8002.';
      }
    } catch {
      result.message = 'Cannot reach Corvus. Make sure Corvus is running on port 8002.';
    }
    ...
    <>
      <section className="about-section">
        ...
        <p style={{ lineHeight: 1.7, marginBottom: 12 }}>
          ...
          These observations flow into Corvus's neuron graph via the Observation Review pipeline.
        </p>
        <p style={{ lineHeight: 1.7 }}>
          Corvus is fully integrated into Corvus — there is no separate backend to run.
          The only external component is a <strong>Chrome extension</strong> that captures screen frames
          and sends them to Corvus. The extension runs in ChromeOS proper (where it can see your displays),
          while Corvus handles capture processing, OCR, and AI interpretation.
        </p>
      </section>
      ...
      <section className="about-section">
        ...
        <table className="about-table" style={{ fontSize: '0.85rem' }}>
          ...
          <tbody>
            ...
            <tr><td>Anthropic API key</td><td>Same key used for Corvus (Haiku for interpretations)</td></tr>
          </tbody>
        </table>
      </section>

      <section className="about-section">
        <h3>Step 1: Verify Corvus is Running</h3>

        <StepCard number={1} title="Ensure Corvus backend is running">
          <p>
            Corvus is integrated into Corvus — no separate backend needed.
            Just make sure Corvus is running on port 8002:
          </p>
...
# Should return: {"status":"ok","service":"corvus","version":"0.3.0"}`}</CodeBlock>
      ...
      <section className="about-section">
        ...
        <StepCard number={5} title="Load the extension">
          ...
          <p style={{ marginTop: 8 }}>
            ...
            It connects to port 8002 (Corvus) for all capture and status endpoints.
          </p>
        </StepCard>
        ...
        <StepCard number={7} title="Open the Corvus dashboard">
          ...
          <p style={{ marginTop: 8 }}>
            Access the Corvus dashboard from the Corvus UI under{' '}
      ...
      <section className="about-section">
        ...
        <StepCard number={8} title="Select a screen to capture">
          ...
          <p style={{ marginTop: 8 }}>
            Corvus captures frames periodically and sends them to Corvus at{' '}
        ...
        <StepCard number={9} title="Configure interpretation settings">
          <p>
            Adjust these settings in the Corvus dashboard or via the Corvus Screen Watcher page:
          </p>
          <ul style={{ margin: '4px 0 0 16px', lineHeight: 1.8 }}>
      ...
      <section className="about-section">
        ...
        <StepCard number={10} title="Configure Corvus enrichment settings">
          <p>
            Since Corvus is integrated into Corvus, observations flow into the neuron graph automatically.
            Go to <strong>Corvus &gt; Screen Watcher</strong> and click the{' '}
            <strong>Corvus Link</strong> tab to control enrichment behavior:
          </p>
          <ul style={{ margin: '4px 0 0 16px', lineHeight: 1.8 }}>
        ...
        <StepCard number={11} title="Verify the observation pipeline">
          <p>
            Once Corvus generates a digest with Corvus integration enabled, it will{' '}
...
┌────────────────────────┐          ┌────────────────────────┐
│  desktopCapture API    │          │  Corvus (port 8002) │
│  ──────────────────    │  JPEG    │                        │
        ...
        <table className="about-table" style={{ fontSize: '0.85rem' }}>
          ...
          <tbody>
            ...
            <tr>
              ...
              <td>Corvus not running on port 8002</td>
              <td>Start Corvus and verify with <code>curl http://localhost:8002/corvus/health</code></td>
            </tr>
            <tr>
            ...
            <tr>
              <td>No observations in Corvus</td>
              <td>Enrichment not enabled</td>
              <td>Check <strong>Corvus &gt; Screen Watcher &gt; Corvus Link</strong> tab</td>
            </tr>
          </tbody>
        </table>
      </section>
...
function TrainingWalkthrough() {
  return (
    <>
      <section className="about-section">
        <h3>Core Concepts</h3>
        <p>Before using Corvus, understand these key ideas:</p>
...
aerospace components under ITAR regulations?`}</CodeBlock>
      ...
      <section className="about-section">
        <h3>Walkthrough: Using Corvus via MCP</h3>
...
claude`}</CodeBlock>
          <p style={{ marginTop: 8 }}>
            Claude Code automatically discovers <code>.mcp.json</code> and spawns the Corvus MCP server
