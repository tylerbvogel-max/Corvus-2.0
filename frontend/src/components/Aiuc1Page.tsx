export default function Aiuc1Page() {
  ...
  const dataPrivacyItems: AiucItem[] = [
    ...
    { id: 'A004', title: 'Protect IP & Trade Secrets', status: 'not-applicable',
      detail: 'Marked N/A \u2014 this requirement maps exclusively to EU AI Act Article 72 with no corresponding NIST AI RMF or ISO 42001 control. Corvus does not scope to EU AI Act compliance.',
  ...
  const securityItems: AiucItem[] = [
    ...
    { id: 'B006', title: 'Prevent Unauthorized AI Agent Actions', status: 'addressed',
      detail: 'Corvus is a retrieval system, not an agentic system. It does not make tool calls, execute code, or take actions. The only external call is the Anthropic API for text generation. Neuron refinements require explicit human approval.',
      source: { nist: 'MAP 2.1' } },
  ...
  const reliabilityItems: AiucItem[] = [
    ...
    { id: 'D003', title: 'Restrict Unsafe Tool Calls', status: 'not-applicable',
      detail: 'Corvus is a retrieval system. It does not make tool calls, execute code, access filesystems, or perform actions. The only external interaction is read-only API calls to Anthropic for text generation.',
  ...
  return (
    <div className="security-page">
      ...
      <p className="security-intro" style={{ marginTop: 8, fontSize: '0.8rem', color: 'var(--text-dim)' }}>
        Note: Corvus is a <em>retrieval</em> system (classify &rarr; score &rarr; assemble &rarr; execute), not an
      ...
      {renderSection(
        'B. Security',
        'Preventing unauthorized access and adversarial exploitation through robustness testing, input filtering, access controls, and monitoring. This is the largest gap area for Corvus.',
        securityItems
      )}
      ...
      {renderSection(
        'D. Reliability',
        'Preventing unreliable outputs that could cause harm, particularly hallucinations and unauthorized tool calls. Tool call requirements are N/A for Corvus\u2019s retrieval architecture.',
        reliabilityItems
      )}

      {renderSection(
        'E. Accountability',
        'Enforcing governance and oversight via failure planning, activity logging, change documentation, transparency reporting, and quality management. This is Corvus\u2019s strongest domain.',
        accountabilityItems
      )}
