...
async def _should_fetch_ygg_context(ocr_text: str) -> bool:
    """Decide whether to query Corvus based on enrich_mode and content heuristics."""
...
async def _fetch_ygg_context(query_text: str) -> str | None:
    """Fetch domain context from Corvus (same process, via internal call)."""
    try:
        import httpx
        ...
        context = f"=== DOMAIN KNOWLEDGE (from Corvus — {neurons_activated} neurons) ===\n"
        if intent:
        ...
        print(f"[Corvus] Corvus context fetch failed: {e}")
        return None


def _build_system_prompt(base_prompt: str, ygg_context: str | None = None) -> str:
    """Assemble system prompt with effort suffix, user context, Corvus context, and attention steering."""
    ...
    # Corvus domain context injection
    if ygg_context:
...
async def _run_text_interpretation(trigger_reason: str) -> str | None:
    ...
    # Fetch Corvus domain context if applicable
...
async def _run_vision_interpretation(trigger_reason: str) -> str | None:
    ...
    # Collect OCR text from images for Corvus context check
...
async def generate_digest(hours: float = 8.0) -> str | None:
    ...
    try:
        ...
        # Submit digest as observation to Corvus
        if interpreter_state.ygg_enabled and digest_text:
            try:
                ...
                print(f"[Corvus] Corvus digest submission error: {e}")
