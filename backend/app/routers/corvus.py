"""Corvus screen-watcher endpoints — integrated into Corvus."""
...
@router.get("/settings/corvus")
async def get_corvus_settings():
...
@router.post("/settings/corvus")
async def set_corvus_settings(
    enabled: str = Form(""),
    url: str = Form(""),
    project_path: str = Form(""),
    enrich_mode: str = Form(""),
):
    """Update Corvus integration settings for context enrichment."""
    if enabled:
...
async def test_ygg_context(text: str = Form("reviewing FAR 52.219 subcontracting plan")):
    """Test Corvus context retrieval with a sample text query."""
