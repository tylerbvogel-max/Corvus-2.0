...
Corvus's neuron graph currently treats all knowledge as homogeneous operational content. This architecture adds source-typed neurons, external reference detection, emergent (self-growing) neurons, and gap analytics. The full design spec is at `readables/regulatory-neuron-spec.md`.
...
**3. Run migration** — Manual ALTER TABLE statements against `corvus.db`. See spec for SQL.
